...
, Messagefrom app.models import Channel
from app.forms import ServerForm

channel_routes = Blueprint("channel", __name__)

@channel_routes.route("/<int:id>")
@login_requiredex(def d)
    ...
    return {"channel'":  , messages: ""channe    return {"channel": one_channel, "messages": channel_messages}, 200
...
def channel_index(id):
    ...
    one_channel = channel.to_dict()
    one_channel_messages = Message.query.filter(Message.channel_id==id).order_by(Message.created_at).all()
    channel_messages = [message.to_dict() for message in one_channel_messages]
