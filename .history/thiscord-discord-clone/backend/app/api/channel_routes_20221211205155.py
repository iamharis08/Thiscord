...
def one_channel_index(id):
    ...
    one_channel_messages = Message.query.join(User).filter(Message.channel_id==id).order_by(Message.created_at.asc()).all()

    channel_messages = [message.to_dict() for message in one_channel_messages]
    print(channel_messages, 'TRYING TO JOIN USER TO MESSAGES !!!!!!!!!!')
    for message in channel_messages:
        print(message.username, "USER MESSAGES")
